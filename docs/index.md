...
Welcome to the VoluLab User Guide.

VoluLab is an open source, browser-based 3D Gaussian Splat Editor. You can use it to view, inspect, transform, combine, crop, clean up and optimize 3D Gaussian Splats.

## Running VoluLab

VoluLab is a web app. There is no hosted instance yet, so run it locally:
...
You can also install VoluLab as a PWA (Progressive Web App). This will make VoluLab appear and behave more like a native application. An app icon for VoluLab will be generated on your desktop or home screen. Furthermore, .ply files will be associated with the VoluLab PWA, enabling you to launch VoluLab more quickly.

## Loading Splats

VoluLab loads splats from .ply files. Only .ply files containing 3D Gaussian Splat data can be loaded. If you attempt to load any other type of data from a .ply file, it will fail.

There are three ways that you can load a .ply file:

1. Drag and drop one or more .ply files from your file system into VoluLab's client area.
2. Select the `Scene` > `Open` menu item and select one or more .ply files from your file system.
3. Use the `load` query parameter. This is in the form: `http://localhost:3000?load=<PLY_URL>`. An example would be:

    http://localhost:3000?load=https://raw.githubusercontent.com/willeastcott/assets/main/dragon.compressed.ply
...
VoluLab can also export to two additional formats via the `Scene` > `Export` sub-menu:
...
The camera controls in VoluLab are as follows:
...
Cropping splats or deleting unwanted Gaussians is a key function of VoluLab. To help with this, there are 3 selection tools available:
...
VoluLab can translate, rotate and scale splats. To do this, select a splat in the Scene Manager and activate one of the gizmos via the horizontal icon bar.
