...
const sanitizeFilename = (filename: string) => {
    ...
    return sanitized.length > 0 ? sanitized : 'volulab';
};
...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    const baseFilename = () => {
        ...
        const source = docName || (splats[0]?.name ?? 'volulab');
    ...
    events.function('render.video', (videoSettings: VideoSettings, fileStream: FileSystemWritableFileStream) => {
        const renderImpl = async () => {
            ...
            try {
                ...
                const capture360 = async (frameTime: number) => {
                    // snapshot the animated camera pose. volulab cameras
        ...
        // codec reclamation.
        if (navigator.locks) {
            return navigator.locks.request('volulab-video-render', renderImpl);
        }
        return renderImpl();
    });
};
